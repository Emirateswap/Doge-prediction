...
const Header = styled(CardHeader)<{ ifoId: string; $isCurrent?: boolean }>`
  ...
  background-image: ${({ ifoId }) => `url('/doge/images/ifos/${ifoId}-bg.svg'), url('/doge/images/ifos/${ifoId}-bg.png')`};
...
const NoHatBunny = ({ isLive, isCurrent }: { isLive?: boolean; isCurrent?: boolean }) => {
  ...
  return (
    <StyledNoHatBunny $isLive={isLive} $isCurrent={isCurrent}>
      <img
        src={`/doge/images/ifos/assets/bunnypop-${!isSmallerThanTablet ? 'right' : 'left'}.png`}
        width={123}
        height={162}
        alt="bunny"
      />
